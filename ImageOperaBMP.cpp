...
void ImageOperaBMP::copyImgData(unsigned char *_srcBuf, unsigned char *_destBuf, int bufSize)
{
...
void ImageOperaBMP::binarizeImage(unsigned char *_inImgData, unsigned char *_outImgData, int imgSize, int threshold)
{
...
void ImageOperaBMP::brigthnessUp(unsigned char *_inputImgData, unsigned char *_outImgData, int imgSize, int brightness)
{
...
void ImageOperaBMP::brigthnessDown(unsigned char *_inputImgData, unsigned char *_outImgData, int imgSize, int darkness)
{
    for(int i =0;i<imgSize;i++)
    {
...
void ImageOperaBMP::computeHistogram(unsigned char * _imgData, int imgRows, int imgCols, float hist[])
{
    FILE *fptr;
...
void ImageOperaBMP::computeHistogram2(unsigned char * _imgData, int imgRows, int imgCols, float hist[],const char *histFile)
{
    FILE *fptr;
...
void ImageOperaBMP::equalizeHistogram(unsigned char * _inputImgData, unsigned char * _outputImgData, int imgRows, int imgCols)
{
    int x,y,i,j;
    int histeq[256];
    float hist[256];
...
void ImageOperaBMP::getImageNegative(unsigned char *_inImgData, unsigned char * _outImgData,int imgWidth,int imgHeight)
{
...
void ImageOperaBMP::Convolve2D(int imgRows, int imgCols,
                                 struct Mask *myMask, unsigned char *input_buf, unsigned char *output_buf)
{
...
void  ImageOperaBMP::detectLine(unsigned char *_inputImgData, unsigned char *_outputImgData, int imgCols, int imgRows, const int MASK[][3])
{
...
void ImageOperaBMP::setMask(int mskRows, int mskCols, const int mskData[])
{
    signed char * tmp;
    int requiredSize;
